...
class Root_to_DataFrame_MT(Thread):
    ...
    def __init__(
        ...
        file_name: str,
        get_sipm: bool = False,
        tree_names: Sequence[str] = ["Tree_Laser", "Tree_R11065", "Tree_SiPM"],
    ) -> None:
    ...
    def run(self) -> None:
        ...
        if self.file_type == "root":
            with uproot.open(self.file_name) as file:
                ...
                for tree_name in self.tree_names:
                    workers.append(
                        Thread(
                            target=setattr,
                            args=(self, tree_name, self.get_tree(file, tree_name)),
                        )
                    )
            ...
            raise NotImplementedError(
                f"Loading {self.file_type} files is not supported"
            )
    ...
    def get_tree(self, file: uproot.ReadOnlyFile, tree_name: str) -> pd.DataFrame:
        """
    ...
    def to_dfs(self, save_as: Optional[str] = None):
        """
    ...
    def from_hdf(
        cls,
        file_name: str,
        get_sipm: Optional[bool] = None,
        tree_names: Optional[Sequence[str]] = None,
    ) -> Self:
        """
        ...
        kwargs = {key: value for (key, value) in _args.items() if value}
        instance = cls(file_name, **kwargs)
...
if __name__ == "__main__":
    ...
    """
    dfs = Root_to_DataFrame_MT("../../pmts/datasets/BC0174_SPE1500VLaserTrig_small.root")
    dfs_ups = Root_to_DataFrame_MT("../../pmts/datasets/BC0174_SPE1500VLaserTrigUPS_small.root")
